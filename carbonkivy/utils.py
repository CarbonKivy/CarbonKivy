...
if platform == "android":
    ...
    def apply_insets_controller(window, decor_view, icon_style, inset_controller):
        if inset_controller and "WindowInsetsControllerCompat" in str(
            ...
            # AndroidX compat
            light = icon_style == "Dark"
            inset_controller.setAppearanceLightStatusBars(light)
            inset_controller.setAppearanceLightNavigationBars(light)
        else:
    ...
    if platform == "android":
        ...
        def apply_system_bars():
            ...
            if Build_VERSION.SDK_INT >= 30:
                apply_insets_controller(
                    window, decor_view, icon_style, inset_controller
                )
                window.setStatusBarColor(status_color_int)
                window.setNavigationBarColor(navigation_color_int)
            else:
                apply_legacy_flags(decor_view, icon_style)
                window.setStatusBarColor(status_color_int)
                window.setNavigationBarColor(navigation_color_int)
